...
with open(os.getcwd()+"\quickstart.yaml", "r") as f:
    try:
...
class ToplevelWindow(customtkinter.CTkToplevel):
    ...
    def load_config_frame(self, root):
        ...
        # Top Buttons
        import_config = customtkinter.CTkButton(config_topbar, text="Import",
                                               command=lambda: self.browse(root))
        import_config.pack(side="left", anchor="nw", padx=5, pady=5)

        # Scrollable Frame
        scrollable_frame = customtkinter.CTkScrollableFrame(config_midbar, label_text="Create A YAML Configuration:",
        ...
        # Bottom Buttons
        apply_button = customtkinter.CTkButton(config_botbar, text="Save", command=lambda: self.read_entries(root))
